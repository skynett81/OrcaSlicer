...
function build_slicer() {
    ...
    for _ARCH in x86_64 arm64; do
        ...
        (
            ...
            rm -rf ./ForgeSlicer.app
            # fully copy newly built app
            cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/ForgeSlicer.app" ./ForgeSlicer.app
            # fix resources
            resources_path=$(readlink ./ForgeSlicer.app/Contents/Resources)
            rm ./ForgeSlicer.app/Contents/Resources
            cp -R "$resources_path" ./ForgeSlicer.app/Contents/Resources
            # delete .DS_Store file
            find ./ForgeSlicer.app/ -name '.DS_Store' -delete
            
            # Copy ForgeSlicer_profile_validator.app if it exists
            if [ -f "../src$BUILD_DIR_CONFIG_SUBDIR/ForgeSlicer_profile_validator.app/Contents/MacOS/ForgeSlicer_profile_validator" ]; then
                echo "Copying ForgeSlicer_profile_validator.app..."
                rm -rf ./ForgeSlicer_profile_validator.app
                cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/ForgeSlicer_profile_validator.app" ./ForgeSlicer_profile_validator.app
                # delete .DS_Store file
                find ./ForgeSlicer_profile_validator.app/ -name '.DS_Store' -delete
            fi
        )

        # extract version
        # export ver=$(grep '^#define FORGE_VERSION' ../src/libslic3r/libslic3r_version.h | cut -d ' ' -f3)
        # ver="_V${ver//\"}"
        # echo $PWD
        # if [ "1." != "$NIGHTLY_BUILD". ];
        # then
        ...
        # zip -FSr OrcaSlicer${ver}_Mac_${_ARCH}.zip ForgeSlicer.app
...
function build_universal() {
    ...
    PROJECT_BUILD_DIR="$PROJECT_DIR/build/$ARCH"
    ARM64_APP="$PROJECT_DIR/build/arm64/OrcaSlicer/ForgeSlicer.app"
    X86_64_APP="$PROJECT_DIR/build/x86_64/OrcaSlicer/ForgeSlicer.app"

    mkdir -p "$PROJECT_BUILD_DIR/OrcaSlicer"
    UNIVERSAL_APP="$PROJECT_BUILD_DIR/OrcaSlicer/ForgeSlicer.app"
    rm -rf "$UNIVERSAL_APP"
    cp -R "$ARM64_APP" "$UNIVERSAL_APP"

    echo "Creating universal binaries for ForgeSlicer.app..."
    lipo_dir "$UNIVERSAL_APP" "$X86_64_APP"
    echo "Universal ForgeSlicer.app created at $UNIVERSAL_APP"

    # Create universal binary for profile validator if it exists
    ARM64_VALIDATOR="$PROJECT_DIR/build/arm64/OrcaSlicer/ForgeSlicer_profile_validator.app"
    X86_64_VALIDATOR="$PROJECT_DIR/build/x86_64/OrcaSlicer/ForgeSlicer_profile_validator.app"
    if [ -d "$ARM64_VALIDATOR" ] && [ -d "$X86_64_VALIDATOR" ]; then
        echo "Creating universal binaries for ForgeSlicer_profile_validator.app..."
        UNIVERSAL_VALIDATOR_APP="$PROJECT_BUILD_DIR/OrcaSlicer/ForgeSlicer_profile_validator.app"
        ...
        echo "Universal ForgeSlicer_profile_validator.app created at $UNIVERSAL_VALIDATOR_APP"
    fi
}
