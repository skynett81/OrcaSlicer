...
if [[ -n "${BUILD_ORCA}" ]] || [[ -n "${BUILD_TESTS}" ]] ; then
    echo "Configuring 3DPrintForge Slicer..."
...
"${BUILD_ARGS[@]}"
	...
	echo "Building 3DPrintForge Slicer ..."
	print_and_run cmake --build $BUILD_DIR --config "${BUILD_CONFIG}" --target ForgeSlicer
	echo "Building ForgeSlicer_profile_validator .."
	print_and_run cmake --build $BUILD_DIR --config "${BUILD_CONFIG}" --target ForgeSlicer_profile_validator
