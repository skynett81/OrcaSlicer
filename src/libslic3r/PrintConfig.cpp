...
void PrintConfigDef::init_common_params()
{
    ...
    def->tooltip = L("3DPrintForge Slicer can upload G-code files to a printer host. This field should contain "
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->tooltip = L(
        ...
        "set appropriately. 3DPrintForge Slicer ensures that adaptive_bed_mesh_min/adaptive_bed_mesh_max values do not exceed these min/max "
    ...
    */
    ...
    def->tooltip = L("After a tool change, the exact position of the newly loaded filament inside "
                     ...
                     "Before purging the print head into an infill or a sacrificial object, 3DPrintForge Slicer will always prime "
    ...
    def->tooltip = L("Connect an infill line to an internal perimeter with a short segment of an additional perimeter. "
                     ...
                     "3DPrintForge Slicer tries to connect two close infill lines to a short perimeter segment. If no such perimeter segment "
    ...
    def->tooltip = L("3DPrintForge Slicer can upload G-code files to a printer host. This field must contain "
    ...
    // ---- Mixed-color / "Full Spectrum" dithering (ported from Snapmaker Orca) ----
    ...
    def->category = L("Others");
    def->tooltip = L("Enable height-weighted cadence for mixed filaments. When disabled, layer-cycle cadence is used.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Others");
    def->tooltip = L("Show and apply the per-row mixed filament Bias control. When enabled, the selected "
                     "filament in a mixed pair is recessed slightly so the other component becomes more visible.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Others");
    def->tooltip = L("Merge ordinary mixed-filament painted regions into a single area when they resolve to the "
                     "same physical filament on a layer.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(true));
    ...
    def->category = L("Others");
    def->tooltip = L("Experimental. Apply Local-Z thinning across whole mixed-color regions instead of limiting "
                     "the effect strictly to painted mixed masks. Only available when Subdivide Mix Layer is enabled.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Material");
    def->tooltip = L("Experimental. When Subdivide Mix Layer is enabled, also apply the same subdivision to infill "
                     "inside mixed-color areas.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Others");
    def->tooltip = L("Experimental. For mixed rows with 3 or more physical filaments, allocate Local-Z sublayers "
                     "directly across all components with carry-over error between layers instead of collapsing them into pair cadence.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->tooltip = L("If you want to process the output G-code through custom scripts, "
                   ...
                   "and they can access the 3DPrintForge Slicer config settings by reading environment variables.");
    def->gui_flags = "serialized";
    def->multiline = true;
    def->full_width = true;
    def->height = 6;
...
//    def->min = 0;
    ...
    def->tooltip = L("On every colour change the nozzle is purged, and that purged filament is "
        ...
        "• Low waste — Balanced plus skipping infill retraction to trim a little more. "
        "Check the result on detailed models.\n\n"
        "(To also purge into a sacrificial object, mark that object's 'Flush into "
        "this object' yourself — it is not enabled automatically, as it would make "
        "ordinary objects receive purge in the wrong colour.)\n\n"
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    def->tooltip = L("This version of 3DPrintForge Slicer may not understand configurations produced by the newest 3DPrintForge Slicer versions. "
                     "For example, newer 3DPrintForge Slicer may extend the list of supported firmware flavors. One may decide to "
    ...
    def->tooltip = L("If enabled, the command line arguments are sent to an existing instance of GUI 3DPrintForge Slicer, "
                     "or an existing 3DPrintForge Slicer window is activated. "
...
ReadWriteSlicingStatesConfigDef::ReadWriteSlicingStatesConfigDef()
{
    ...
    def->tooltip = L("Position of the extruder at the beginning of the custom G-code block. If the custom G-code travels somewhere else, "
                     "it should write to this variable so 3DPrintForge Slicer knows where it travels from when it gets control back.");
    ...
    def->tooltip = L("Retraction state at the beginning of the custom G-code block. If the custom G-code moves the extruder axis, "
                     "it should write to this variable so 3DPrintForge Slicer de-retracts correctly when it gets control back.");
