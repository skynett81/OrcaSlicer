...
// Apply a (possibly pre-processed) MM segmentation to the print object's layer
// regions. The segmentation is generated by the caller so the mixed-color /
// Local-Z dithering pipeline can interpose between generation and application
// (ported from Snapmaker Orca).
...
void PrintObject::slice_volumes()
{
    ...
    if (const auto& volumes = this->model_object()->volumes;
        ...
        // Generate the MM segmentation first, so the mixed-color / Local-Z
        // dithering pipeline (Full Spectrum) can interpose here before it is
        // applied. (Ported seam from Snapmaker Orca; chain added in phase 5b.)
