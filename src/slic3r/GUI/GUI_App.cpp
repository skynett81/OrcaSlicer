...
private:
    ...
    wxString m_text_version = "v" + GUI_App::format_display_version();
...
//	SetAppDisplayName(SLIC3R_APP_NAME);
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current 3DPrintForge Slicer Version %1% build %2%") % FORGE_VERSION % GIT_COMMIT_HASH;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "3DPrintForge Slicer", wxICON_QUESTION | wxYES_NO);
...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #009789  -- 73
        if (bg_col != orig_col) {
    ...
    http.header("accept", "application/vnd.github.v3+json")
        ...
        .on_complete([this, by_user, check_stable_only](std::string body, unsigned http_status) {
          ...
          try {
            ...
            Semver    current_version = get_version(FORGE_VERSION, matcher);
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" Orca.Slicer.1";
    std::wstring prog_desc = L"3DPrintForge Slicer";
