        ...
        try {
            if (type_3mf) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    const wxString msg_unsupported_geometry    = _L("The 3MF is not supported by 3DPrintForge Slicer, loading geometry data only.");
                    const wxString msg_old_orca_geometry       = _L("The 3MF file was generated by an old 3DPrintForge Slicer version, loading geometry data only.");
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (file_version <= orca_tag_start_version) {
                            ...
                            if (load_config && config_loaded.empty()) {
                                ...
                                wxString text = _L("The 3MF was created by BambuStudio. Some settings may differ from 3DPrintForge Slicer.");
                                log_and_show_3mf_info(text, bambu_project_title);
                            }
                        }
                    }
...
#else
    wxGetApp().mainframe->SetTitle(m_project_name + wxString::Format(" - %s", SLIC3R_APP_NAME));
...
void Plater::priv::update_title_dirty_status()
{
    ...
    wxGetApp().mainframe->update_title_colour_after_set_title();
#else
    wxGetApp().mainframe->SetTitle(title + wxString::Format(" - %s", SLIC3R_APP_NAME));
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        if (device_page) {
            ...
            content = wxString::Format(
                _L("%s can't connect to %s. Please check if the printer is powered on and connected to the network."),
                SLIC3R_APP_NAME, printer_name);
        }
