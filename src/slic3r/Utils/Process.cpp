...
// For starting another 3DPrintForge Slicer instance on OSX.
...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance)
{
	...
	path += "\\";
	path += (instance_type == NewSlicerInstanceType::Slicer) ? "3dprintforge-slicer.exe" : "bambu-gcodeviewer.exe";
	...
	// Don't call with wxEXEC_HIDE_CONSOLE, 3DPrintForge Slicer in GUI mode would just show the splash screen. It would not open the main window though, it would
...
#else // Linux or Unix
	{
		...
		if (args.empty()) {
			...
			my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "3dprintforge-slicer" : "bambu-gcodeviewer")).string();
